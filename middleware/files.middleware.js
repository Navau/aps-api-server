    ...
    map(filesUploaded, async (item, index) => {
      ...
      if (isEndRegisterAllFilesAux === false) {
        if (!item.originalname.includes(fechaOperacion)) {
          ...
          const arrayDataObject = await formatearDatosEInsertarCabeceras(
            headers,
            dataSplit
          );
      ...
      await insertFilesPromise
        .then(async (response) => {
          if (errors.length >= 1 || errorsCode.length >= 1) {
            const insertErrorsPromise = new Promise(async (resolve, reject) => {
              let queryFiles = "";
              let bodyQuery = [];
              let currentFiles = [];
              let resultsPromise = [];
              map(errors, (item, index) => {
                bodyQuery.push({
                  ...
                  fila: item.hasOwnProperty("fila")
                    ? parseInt(item.fila) + 1
                    : 0,
                  columna: item.hasOwnProperty("columna") ? item.columna : 0,
                });
              });
